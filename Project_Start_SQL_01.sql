SELECT * FROM order_items_dataset;
	...
	CREATE TABLE cubo_pedidos AS
		...
		FROM order_items_dataset t1
					INNER JOIN orders_delivery_dataset t2 ON t1.order_id = t2.order_id
						INNER JOIN products_dataset t3 ON t1.product_id = t3.product_id
							INNER JOIN sellers_dataset t4 ON t1.seller_id = t4.seller_id